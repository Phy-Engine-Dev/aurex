...
class InspectionRecoveryTests(unittest.TestCase):
    ...
    def test_hex_queries_m00008_through_m00022_all_execute_in_reviewed_batches_same_task(self):
        outputs, verdicts = [], []
        for batch in range(9):
            outputs.extend(call(f'bad-{i}', f'm000{i+8:02x}', focus=i % 2 == 0)
                           for i in range(batch * 3, batch * 3 + 3))
            outputs.append(support.reply('Reassess the actual lookup evidence; no functional verdict.'))
            verdicts.append({'outcome': 'continue', 'answer': '使用已记录的真实ID继续定位，不要盲目枚举。'})
        outputs += [call('actual', 'real-id'), support.reply('Only component lookup was completed, not simulation.')]
        verdicts.append({'outcome': 'completed', 'answer': 'Only component lookup was completed, not simulation.'})
        agent, fake, execute = self.fixture(outputs, [ToolError(NOT_FOUND)] * 27 + [{'components': [{'id': 'real-id'}]}])
        fake.final_reviews = iter(verdicts)
        result = agent.handle(user_text='Inspect the requested component')
        self.assertEqual(result['status'], 'completed')
        self.assertEqual(execute.call_count, 28)
        self.assertEqual(len(self.recoveries(agent, result)), 9)
        ...
        self.assertEqual(execute.call_args_list[26].args[1]['focus_ids'], ['m00022'])
        execution_requests = [(m, o) for m, o in fake.requests if m[0]['content'] != FINAL_SYSTEM]
        self.assertEqual(sum(not o['tools'] for _, o in execution_requests), 0)
    ...
    def test_clarification_never_enables_inspection_even_after_continue(self):
        ...
        self.assertEqual(result['answer'], '你指的是哪条内容？')
