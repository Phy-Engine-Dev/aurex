...
class DataModeNativeTests(unittest.TestCase):
    ...
    def test_interface_all_three_sources_paginates_only_actual_io(self):
        ...
        for source in (created["sav_path"], created["circuit_path"], analyzed["state_path"]):
            ...
            self.assertEqual(first["array_order"], "saved_component_order")
            self.assertEqual(first["ref_semantics"], "generated_locator_not_bit_index")
            self.assertIsNone(first["logical_bit_order"])
            self.assertEqual(first["logical_bit_order_status"], "undeclared")
