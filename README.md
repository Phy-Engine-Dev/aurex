...
每个用户问题先读取原帖标题、完整正文、评论、作者与时间、回复关系和原始分类/状态；封面和用户上传图片先归档为可读取的路径，不自动把像素送入模型。AI需要视觉信息时必须显式调用 `view_image` 或支持图片的工具并设置 `with_image=true`。每个用户问题的第一条全上下文模型请求传 `chat_template_kwargs.enable_thinking=true`；后面的工具调用、工具结果分析和候选答复整理传 `false`。发布仍走独立证据审核；普通最终答复由同一个执行Agent直接完成，不插入会把已完成任务重新打开的审核/`continue`循环。容器默认关闭思考，由客户端逐次设置。上下文摘要请求默认关闭思考。`reasoning_content` 只进入单独的跟踪事件，不作为 `assistant.content` 回放，也不写入公开作品介绍。
...
工作台默认绑定 `0.0.0.0:4097`，本机及 Tailscale 可访问 `http://100.123.133.75:4097`。管理员访问码在 `.config/web-token`。普通用户的数据按“浏览器能力凭据 + 关联用户 ID”隔离；裸用户 ID 不能跨设备恢复或接管会话。网页支持带图片提问、选择 Experiment/Discussion/User 目标、逐步查看工具结果和图片、下载本地实验文件。选中会话后可点“导出会话”；CLI 使用 `/export [目标路径]`（也兼容 `/exportsection`）。仍建议只在可信内网或 Tailscale 使用，不要把 HTTP 端口暴露到不可信公网。
...
社区来源必须绑定真实提问者ID：发布正文末行与最终回复开头分别由服务器添加 `<user=ID>@nickname</user>`，不能让模型猜ID或改提问者。Web/管理员本地任务不伪造提问者、不添加该mention。正常任务由同一个执行Agent持续到最终答复，不再插入会把已完成任务重新打开的独立审核/`continue`循环；服务器仍以持久化状态和外发账本保证至多一次最终回复。`dry_run` 不产生社区外写。
...
上下文管理不是任务总token或工具轮数预算。v3任务不会因为沿用旧 `agent.max_tool_loops` 达到固定轮数而收尾；`llm.max_output_tokens=null` 时，第一次完整上下文思考不再被4096/32000等人为上限截断，API按真实剩余窗口处理。后续无思考工具导航单轮使用2048 token、生成活动60秒的边界；排队和prefill时间不计入，若实际是在写最终正文，则关闭工具后由同一Agent做一次不受该导航边界影响的完整答复。显式配置值仍生效。单次响应 `length`、极端机械重复或只有私有思考而无工具/答案时，归档不完整输出且**不执行半截工具调用**；普通无效交接最多两次聚焦恢复。新的真实工具证据会恢复普通额度；没有新证据的第三次失败只给一次有界 `INCONCLUSIVE`。网络故障、非法响应、用户取消、1800秒可配置硬上限与真正阻碍仍明确记录。授权发布任务没有成功回执时保持 `needs_attention`，不能只因模型说“完成”就标成功。

重复查询和复测始终允许，工具不会因为过去调用过而被禁用；但相同结果不会被当作“新证据”重置生成恢复。任务计划仅作为持久化导航，同一个执行Agent依据真实工具结果继续、完成或报告阻碍，不插入第二个审核模型，也不把求解器失败当成测量结果。任务总时长仍由 `agent.task_timeout_sec` 控制，默认且最大为1800秒。

借鉴OpenCode的预算与压缩机制：预留输出和安全空间，优先调用vLLM tokenizer计算真实输入长度；接近阈值时压缩旧对话、保留完整工具调用/结果对及最近内容。长原文先分块摘要，原文与工具全文始终保存在SQLite供操作员审计；模型使用标题、正文、电路和HDL的有界专用读取器，不能用通用 `read_context/read_content` 重放巨大归档。不会把所有历史思考重新塞入模型。
...
`llm.reasoning_effort` 默认 `null`，请求不传该字段而保留模板默认；可选 `low` / `medium` / `xhigh`。当前Qwen模板默认xhigh，medium仍开启思考而不额外注入聚焦提示，low会要求思考简短且聚焦；2×V100上的Aurex部署示例使用low，让首轮继续思考但减少在同一推导分支上来回重算。这是模板文本控制，不是硬token配额，也不是质量保证。首轮thinking请求不覆盖模型随checkpoint发布的原生采样温度。`llm.thinking_frequency_penalty` 是仅作用于thinking请求的实验开关，默认0且不发送；实测0.3会让Qwen3.8在数独回放中产生超过4.9万字符的重复推演，因此不能作为默认防循环手段。后续no-thinking工具导航和摘要仍只使用`llm.temperature`。这些设置都不改变1800秒任务硬上限、Agent层级或上下文整理策略。
...
会话导出不暂停正在运行的 Agent。服务从同一个 WAL 只读事务取得已提交数据的一致时点，将该会话在14张关联表中的记录复制到新建SQLite，校验引用、`quick_check`、空闲页和压缩包CRC后才提供下载。最终文件固定为只含一个SQLite的 `ZIP_DEFLATED` 压缩包；未压缩中间文件始终位于权限0700的临时目录、文件权限0600，成功、失败或服务关闭时都会清理，绝不会退回裸SQLite。一次性下载票据默认10分钟有效，成功下载即失效；普通用户只能导出自己的会话，管理员可导出全部会话。管理员包包含内部审计链；普通用户包清除所有权哈希、子Agent私有交接与私有消息正文，只保留用户可见的子任务状态、报告和脱敏后的工具证据关系，避免绕过Web权限边界。导出保留数据库中的artifact索引，但不捆绑其外部文件，也不包含独立发布凭据库、HDL工作区数据库或缓存文件。
