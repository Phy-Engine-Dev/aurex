...
def _compact_view(data: dict[str, Any], summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"netlist": {"components": compact, "nodes": nodes,
...
def _interface_result(result: dict[str, Any], offset: int, limit: int) -> dict[str, Any]:
    ...
    return {"interface_only": True, "with_image": False, "images": [], "ports": ports[offset:offset + limit],
            ...
            "scope": "Actual Logic Input/Output devices in saved order, with original IDs/labels/positions and exact saved-node connection counts; internal gates omitted. A port with connected_to_other_components=false is isolated and cannot stimulate or observe the circuit. This is not an inferred module signature: unlabelled inputs may be constants. No stimulus or waveform is invented.",
...
def circuit_inspect(runtime: ToolRuntime, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if listing_only:
        offset, limit = 0, 8
    default_view = "spatial" if (any(args.get(k) for k in ("focus_id", "focus_ids", "query")) or args.get("offset", 0) > 0) else "auto"
    ...
    if value.get("schema") == "aurex.circuit.v1":
        spec = normalize_spec(value, digital_component_limit=runtime.config.phy_engine.digital_component_limit)
...
def circuit_query_many(runtime: ToolRuntime, args: dict[str, Any]) -> dict[str, Any]:
    """Inspect several exact circuit targets in one model/tool round trip.

    This deliberately reuses circuit_inspect's authoritative matching instead
    of teaching a second implementation how PLSAV refs, identifiers and nodes
    work. Images stay disabled and each result is projected to the matched
    rows, so one batch does not duplicate whole-netlist artifacts in context.
    """
    path = args.get("path")
    ...
    if len(set(normalized)) != len(normalized):
        raise ToolError("queries must be unique")
    ...
    for query in normalized:
        ...
        for component in netlist.get("components", []):
            ...
            if component_id in component_catalog:
                continue
            compact = {key: copy.deepcopy(component[key]) for key in
                       ("id", "ref", "type", "label", "properties") if key in component}
            if not exact_node_query:
                compact["pins"] = []
                for pin in component.get("pins", []):
                    ...
                    compact["pins"].append(projected)
            native = component.get("native")
            if isinstance(native, dict):
                native_type = native.get("type")
                compact["native_type"] = native_type
                measurements = native.get("measurements")
                if isinstance(measurements, dict):
                    keys = (("digital", "model_state", "unconnected_pins")
                            if isinstance(native_type, str) and native_type.startswith("digital_") else
                            ("voltage", "voltage_imag", "current", "current_imag",
                             "model_state", "effective_params"))
                    selected = {key: copy.deepcopy(measurements[key]) for key in keys
                                if key in measurements and measurements[key] not in ([], {}, None)}
                    if selected:
                        compact["measurements"] = selected
            component_catalog[component_id] = compact
        # Component queries already carry every matched pin and its connection
        # count.  Only an exact node query needs an explicit connection list;
        # returning the union of every matched component's neighboring nodes
        # made a 16-display batch larger than the original model turn.
        exact_nodes = []
        for node in netlist.get("nodes", []):
            if str(node.get("id", "")).casefold() != query.casefold():
                continue
            exact_nodes.append({key: copy.deepcopy(node[key]) for key in
                                ("id", "total_connections", "connections", "connections_truncated")
                                if key in node})
        rows.append({
            "query": query, "ok": True,
            "component_ids": component_ids,
            # Keep a minimal identity record inside every independent result.
            # The detailed rows below remain globally deduplicated, but a
            # semantic checkpoint is then still intelligible even if it keeps
            # this query and omits the distant top-level catalog.
            "components": [
                {key: copy.deepcopy(component_catalog[component_id][key])
                 for key in ("id", "ref", "type", "label")
                 if key in component_catalog[component_id]}
    ...
    return {
        ...
        "limit_per_query": limit, "results": rows,
        "component_catalog": list(component_catalog.values()),
        "artifact": artifact, "circuit_path": circuit_path,
        **({"state_path": state_path} if state_path else {}),
        "scope": ("Independent compact circuit_inspect matches. Every successful result is self-contained with minimal "
                  "id/ref/type/label identities, while pins, properties and measurements are deduplicated in component_catalog by ID; "
                  "a failed query does not discard successful siblings. Pins include node connection counts; "
                  "measurements are present for recorded PE state. Exact node queries include their connection list. "
                  "To locate a component class, query the exact returned type substring (for example 'Square Source'); "
                  "do not search or page the full netlist artifact with read_context. The artifact is durable/downloadable "
                  "source evidence, not the model lookup interface; this result is read-only, not simulation."),
    }
...
def register_circuit_tools(registry: ToolRegistry) -> None:
    definitions = [
        ...
        ("circuit_read_trace", "Read actual timestamped samples from a saved native TR state, without rerun or interpolation. Select up to 8 analog nodes OR native-digital component_ids; never include analog interaction controls such as buttons/switches/rheostats in component_ids. Prefer sample_indices (up to 16 exact zero-based frames, e.g. before/after an interaction and the final frame) for bounded evidence; offset/limit pages at most 64 frames. Pure digital states default to digital IDs. Historical records retain their original values and propagation-policy warning. For the separate stimulus sequence use circuit_read_stimulus, not this TR reader.",
        ...
        ("circuit_inspect", "Inspect original PLSAV, native design or recorded state. Default is data-only, with bounded type counts and examples; with_image=true explicitly generates a diagram. interface_only=true lists digital I/O. controls_only=true lists exact agent-operable analog controls (buttons, switches, selector switches, rheostats, voltage sources) plus digital inputs, their current values and ranges, without an image. For details, focus resolves exact Identifier before ref before unique Label; ambiguous labels error. An exact N<number> query returns up to 8 components connected to that node per page. Full lossless state/netlist remain artifacts. Images preserve positions, never prove fresh simulation.",
         {"path": _PATH, "with_image": _WITH_IMAGE, "interface_only": {"type": "boolean", "default": False, "description": "Data-only digital I/O listing, mutually exclusive with with_image=true and focus/query/camera/view. Original order, default/max 64 per page."},
          ...
          "query": {"type": "string", "description": "Exact N<number> selects all components connected to that node; exact C<number> selects that displayed ref. Otherwise literal component text search. For node/ref lookup use this on the original path, never read_context/find on the full netlist artifact."}, "camera": _CAMERA}, ["path"], circuit_inspect),
        ("circuit_query_many", "Read several circuit refs, identifiers, nodes or literal component queries in one read-only call. Prefer this after interface/control discovery or when a large circuit needs multiple targeted lookups; do not spend one model round trip per C-number. Every result retains minimal id/ref/type/label identity across compaction; detailed pins/properties/measurements use one deduplicated component_catalog. Each query independently reports ok/error, so one missing node does not discard valid siblings. Repeating a query for a concrete recheck is allowed. No images or simulation.",
         {"path": _PATH,
          "queries": {"type": "array", "minItems": 1, "maxItems": 24, "uniqueItems": True,
                      "items": {"type": "string", "minLength": 1, "maxLength": 128},
        ...
        ("circuit_edit", "Create a new revision of a circuit by adding/removing components, changing params, or connecting a pin to a named node. Original stays intact; nothing is published. A no-op update to existing/default values is rejected instead of creating a misleading new revision.",
