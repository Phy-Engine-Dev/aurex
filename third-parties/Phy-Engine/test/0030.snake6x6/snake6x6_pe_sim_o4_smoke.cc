...
int main()
{
    ...
    auto settle = [&] {
        // digital_clk() is one propagation round, not an implicit unbounded
        // combinational fixpoint.  Large synthesized designs must request a
        // propagation budget just like the public TR bridge does.
        for(std::size_t i{}; i < 32u; ++i) { c.digital_clk(); }
    };
