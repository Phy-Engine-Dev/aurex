...
module top(input clk, input rst_n, output y);
  ...
  assign y = state[0] | state[1] | state[2];
    ...
    ::phy_engine::verilog::digital::pe_synth_options opt{
        ...
        // FSM don't-care minimization is a binary truth-table transform.  O4
        // deliberately skips the non-CUDA QM path, so exercise the supported
        // O3 contract with binary inputs instead of asking an inactive pass to
        // change the netlist.
