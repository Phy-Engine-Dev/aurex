...
namespace phy_engine
{
    ...
    public:
                ...
                for(auto c{i.begin}; c != i.curr; ++c)
                {
                    if(c->num_of_analog_node == 0)
                    {
                        if(!c->pins.empty())  // digital
                        {
                            ...
                            {
                                c->node_information.dn.state = ::phy_engine::model::digital_node_statement_t::X;
                                // Run every connected table-driven digital
                                // model at least once. Otherwise a stateful
                                // model whose clock driver starts and remains X
                                // never receives a node-change event, leaving
                                // its valid saved/seeded outputs incorrectly X.
        ...
        bool solve_once() noexcept
        {
            ...
            for(auto const [v, n, driver]: digital_out)
            {
                (void)driver;
