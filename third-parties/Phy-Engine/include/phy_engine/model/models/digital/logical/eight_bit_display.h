...
namespace phy_engine::model
{
    ...
    {
        if(vi.type != ::phy_engine::model::variant_type::d) { return false; }
        if(n == 2) { clip.Ll = vi.d; return true; }
        if(n == 3) { clip.Hl = vi.d; return true; }
        return false;
    }
        ...
        get_attribute_define(::phy_engine::model::model_reserve_type_t<EIGHT_BIT_DISPLAY>, EIGHT_BIT_DISPLAY const& clip, ::std::size_t n) noexcept
    {
        switch(n)
        {
            ...
            case 2: return {.d{clip.Ll}, .type{::phy_engine::model::variant_type::d}};
            case 3: return {.d{clip.Hl}, .type{::phy_engine::model::variant_type::d}};
            default: return {};
        }
    }
