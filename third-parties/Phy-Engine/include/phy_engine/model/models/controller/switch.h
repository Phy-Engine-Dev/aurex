...
namespace phy_engine::model
{
    struct single_pole_switch
    {
        ...
        bool cut_through{};
        // Closed-contact resistance.  Zero preserves the ideal native switch;
        // importers can request a finite contact for source formats whose
        // switches are deliberately non-ideal.
        double r_closed{};
                                               ...
                                               ::phy_engine::model::variant vi) noexcept
    {
        switch(n)
        {
            ...
            case 1:
            {
                if(vi.type != ::phy_engine::model::variant_type::d ||
                   !::std::isfinite(vi.d) || vi.d < 0.0) [[unlikely]] { return false; }
                sps.r_closed = vi.d;
                return true;
            }
            default:
        ...
        get_attribute_define(::phy_engine::model::model_reserve_type_t<single_pole_switch>, single_pole_switch const& sps, ::std::size_t n) noexcept
    {
        switch(n)
        {
            ...
            case 1:
            {
                return {.d = sps.r_closed, .type = ::phy_engine::model::variant_type::d};
            }
            default:
            {
                return {};
        ...
        iterate_dc_define(::phy_engine::model::model_reserve_type_t<single_pole_switch>, single_pole_switch const& sps, ::phy_engine::MNA::MNA& mna) noexcept
    {
        ...
        if(node_0 && node_1) [[likely]]
        {
            ...
            double const r_contact{sps.cut_through ? sps.r_closed : mna.r_open};
