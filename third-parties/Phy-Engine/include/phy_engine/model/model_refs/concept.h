...
namespace phy_engine::model
{
    ...
    // Digital models use Ll/Hl both to decode analog-connected inputs and to
    // drive analog-connected outputs.  Small combinational and sequential
    // primitives historically exposed the fields but no attribute API, so a
    // mixed-signal adapter could not preserve the levels stored in a .sav.
    // This is the fallback attribute contract for models which do not provide
    // a more specific overload.  Non-template model overloads (including
    // INPUT, gates and stateful aggregate displays) remain authoritative.
    template <typename mod>
        requires requires(mod& value) { value.Ll; value.Hl; }
    inline constexpr bool set_attribute_define(model_reserve_type_t<mod>, mod& value,
                                                ::std::size_t index, variant input) noexcept
    {
        if(input.type != variant_type::d) { return false; }
        if(index == 0) { value.Ll = input.d; return true; }
        if(index == 1) { value.Hl = input.d; return true; }
        return false;
    }

    template <typename mod>
        requires requires(mod const& value) { value.Ll; value.Hl; }
    inline constexpr variant get_attribute_define(model_reserve_type_t<mod>, mod const& value,
                                                   ::std::size_t index) noexcept
    {
        if(index == 0) { return {.d{value.Ll}, .type{variant_type::d}}; }
        if(index == 1) { return {.d{value.Hl}, .type{variant_type::d}}; }
        return {};
    }

    template <typename mod>
        requires requires(mod const& value) { value.Ll; value.Hl; }
    inline constexpr ::fast_io::u8string_view get_attribute_name_define(
        model_reserve_type_t<mod>, ::std::size_t index) noexcept
    {
        if(index == 0) { return {u8"Ll"}; }
        if(index == 1) { return {u8"Hl"}; }
        return {};
    }
