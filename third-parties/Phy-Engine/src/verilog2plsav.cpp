...
int main(int argc, char** argv)
{
    ...
    auto const changes_binary_semantics = [&](std::string const& warning) noexcept
    {
        if(assume_binary_inputs)
        {
            // PhysicsLab digital pins and gates are binary.  Under the explicit
            // --assume-binary-inputs contract, === and == have the same truth
            // table and an unspecified input power-on value is represented by
            // the platform's deterministic low state.  Keep strict mode strict
            // for every other X/Z, tri-state, latch or reset degradation.
            if(warning == "pe_to_pl: degrading CASE_EQ -> Xnor Gate (drops X/Z-aware === semantics)" ||
               warning == "pe_to_pl: Logic Input initial state is X/Z; defaulting to 0")
            {
                return false;
            }
        }
    ...
    bool const changed_semantics = ::std::any_of(r.warnings.begin(), r.warnings.end(), changes_binary_semantics);
    if (has_flag(argc, argv, "--strict-export") && changed_semantics)
    {
